...
import com.meiji.utils.NetworkUtils;
...
public class SystemService extends Service {
    ...
    private NetworkConnectChangedReceiver wangluoguangbo = new NetworkConnectChangedReceiver();
    ...
    public void onDestroy() {
        super.onDestroy();
        unregisterReceiver(wangluoguangbo);
    ...
    private void init() {
        ...
        registerReceiver(wangluoguangbo,filter);
      ...
      if (context != null) {
           if (NetworkUtils.isNetworkAvailable(context)) {
                networkCallk.Onnormal(context.getResources().getString(R.string.netok));
          } else {
        ...
        if (mediaPlayer2==null){
            ...
            try {
               // Log.i("测试", "initSoundEffect: ");
                fd1 = getAssets().openFd("anniu.wav");
                fd2 = getAssets().openFd("draft.mp3");
                mediaPlayer2.setDataSource(fd2);
                mediaPlayer2.setLooping(true);//循环
                mediaPlayer2.prepare() ;

                mediaPlayer1.setDataSource(fd1);
